...
2. そのうえで、個人ブログならではのプラスアルファを見出しで足す
   （忖度のない本音、向き不向き、失敗しやすいポイント、判断基準、現場感のある注意点など）
3. 法人ブログが事実を羅列するだけなら、個人ブログは「読者の迷いに踏み込む切り口」を必ず入れる
4. 先に検索意図を整理し、調査メモの答え合わせ結果を反映して構成をブラッシュアップする
5. 読者が迷わず本文を書ける具体性にする（各見出しに書く要点を箇条書き）
6. 一次情報（体験・実測・感想・失敗談）を入れると強い箇所を、構成段階で明示する
...
9. 一次情報を追記すべき箇所（体験談・実測・感想など。本文では捏造禁止なので「要追記」と明記）
...
【個人ブログとしての立ち位置（最重要）】
・法人ブログが事実のみを羅列するのに対し、個人ブログは忖度のない切り口で書く
・それが個人ブログの存在意義。一般論のまとめだけで終わらせない
・向き不向き、デメリット、失敗しやすい点、本音の判断基準を具体的に書く
・煽りや過度なセールストークではなく、読者に正直な伴走感で書く

【文体・文末】
・基本はです・ます調だが、文末を単調にしない
・文末はバリエーション豊かに混ぜる（例：〜です／〜ですよね／〜かも／〜かもしれません／体言止め／〜ですね／〜でしょう）
・感嘆符（！）や疑問符（？）も自然な範囲で使ってよい
・同じ文末が3文以上連続しないようにする
・結論先行（冒頭で検索意図の中心に答える）
・読者に直接語りかける自然な文章にする
...
3. 架空の一人称体験談・架空の口コミ・捏造データ・捏造の一次情報は絶対に書かない
4. 一次情報を入れると良い箇所には、本文中にわかりやすい追記マーカーを残す
   マーカー形式の例（内容に合わせて文言を変えてよい）：
   `[体験談を追記：ここにご自身の実体験・失敗談・感想を書いてください]`
   `[一次情報を追記：実際に試した手順・かかった時間・金額・結果を書いてください]`
   `[実測・観察を追記：ご自身で見た変化や比較メモを書いてください]`
   記事全体で最低3箇所はマーカーを入れる（多すぎて読みにくくならない範囲で）
